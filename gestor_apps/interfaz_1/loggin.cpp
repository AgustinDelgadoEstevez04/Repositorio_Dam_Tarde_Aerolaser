    ...
    , ui(new Ui::loggin)
{
    ...
    connect(ui->usuario, &QLineEdit::returnPressed, this, &loggin::on_iniciar_clicked);
    connect(ui->contrasena, &QLineEdit::returnPressed, this, &loggin::on_iniciar_clicked);
}
...
void loggin::on_registrar_clicked()
{
    ...
    if (nombre.length() < 6) {
        ...
        ui->usuario->setFocus(); // Fix: Focus on username field
        return;
    }
    if (contrasena.length() < 6) {
